...
function JoinRequest(props) {
  const handleAcceptJoinRequest = useMutation(
    async() => {
      ...
      await axios.post(`http://localhost:8000/organization/${props.o_id}/request/${props.r_id}/accept`
    )}, {
      onSuccess: async () => {
  ...
  const handleDenyJoinRequest = useMutation(
    async () => {
      axios.defaults.headers.post["Authorization"] = `Bearer ${props.token}`;
      await axios.post(`http://localhost:8000/organization/${props.o_id}/request/${props.r_id}/decline`)
    },{
      onSuccess: async () => {
        props.requestDeleted(props.r_id)
      }
    ...
    <div className="requests-container">
      <div className="join-request-card">
        ...
        <div className="join-request-card-buttons">
          <Button variant="outline-success" onClick = {() => handleAcceptJoinRequest.mutate()}>Accept</Button>
          <Button variant="outline-danger" onClick = {() => handleDenyJoinRequest.mutate()}>Deny</Button>
        </div>
      </div>
    </div>
  );
